...
if(!isset($data) || !is_array($data)) {
  ...
  // get all tags
  foreach($store as $s) {
    $data = array_merge($data, str::split($s->{'caption_1'}, $separator));
    $data = array_merge($data, str::split($s->{'caption_2'}, $separator));
    $data = array_merge($data, str::split($s->{'caption_3'}, $separator));
    $data = array_merge($data, str::split($s->{'caption_4'}, $separator));
  }
